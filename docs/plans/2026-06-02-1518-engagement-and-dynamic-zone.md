...
> **Status**: Phase 1 + 2 + 3(B-3) 구현 완료 (2026-06-02). B-2(정확도 적응)는 보류.
> 결정: active 중 hold=유한 확장 · idle grace=1200ms · upright 기준 통일(6°) · Phase3=B-3(속도 적응).
...
#### 구현된 hold zone 동작 기준 (검토용 — 2026-06-02)

hold zone(`holdFrac`)은 **"시선이 어디까지 벗어나도 rail lock 을 유지할지"** 의 경계(viewport
수직 방향 비율)다. 그런데 이걸 조절하는 기준은 *시선이 아니라 머리 기울기(operating)* 다 —
gaze 허용 범위를 head 로 제어한다.

- **기준 신호** `operating` (App `operatingRef`, 단일 신호):
  `operating = 얼굴 검출 && |head roll(절대)| > UPRIGHT_MAX_DEG(6°)` = "지금 머리를 기울여 조작 중".
- **늘어남 (0.24 → 0.55, 즉시)**: `operating` 이 참이 되는 순간. 조작 중엔 시선이 결과를 보러
  중앙으로 떠나도 rail 이 안 풀린다.
- **줄어듦 (0.55 → 0.24, ~400ms 점진)**: 머리가 꼿꼿(upright, ≤6°)해지면. 수축 중인 넓은 구간이
  grace 역할. 시상수 `holdZoneDecayMs`.
- 코드: `edge-detector.ts update()` — `targetFrac = operating ? 0.55 : 0.24`,
  확장은 즉시 / 수축은 `holdFrac += (target-holdFrac)*(1-e^(-dt/400))`.
- 보조: 시선이 *현재* holdFrac 를 벗어나도 `exitGraceMs`(250ms) 더 버틴 뒤 해제.

> ⚠️ 재검토 포인트: 기준이 "머리 기울기"뿐이라 *시선만으로는* 영역이 안 변한다. enter 쪽
> (acquire) 동적화(B-2/B-3, Phase 3)와 합쳐 "들어갈 때/나올 때 범위 모두 동적"을 완성할지 검토.
...
- **Phase 3** ✅ B-3 (2026-06-02) — enter zone 속도 적응. B-2(정확도 적응)는 보류.
  - `SnapConfig`: `enterFracMin`(0.12)/`enterFracMax`(0.26)/`enterApproachRefPxs`(600)/`enterLateralRefPxs`(500).
  - `IntentTracker`: primary 의 approach/lateral 속도로 `dynamicEnterFrac` 계산 → in-zone·closeness 에 사용.
    approach↑(곧장 접근) → 확장, lateral↑(가로 스캔) → 수축. `IntentSample.enterFrac` 노출.
  - `edge-detector` snapshot.intentZoneFrac 를 동적 enterFrac 로 노출(EdgeZones/HUD 시각화).
  - DebugHud `enter zone` 행 추가.
  - ⚠️ enter 변경이라 FTR(오발동) 검증 필요 — Evaluation trigger eval 로 확인 권장.
  - (보류) B-2 정확도/노이즈 적응: gaze 분산 추정기 필요, 추후.
