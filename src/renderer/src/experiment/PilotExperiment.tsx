...
export function PilotExperiment({
  ...
  const applyKeyboardVolumeStep = useCallback(
    (direction: CommandDirection): boolean => {
      const target = selectedTargetRef.current
      if (!target) return false
      applyVolume(target, nextValueForDirection(volumesRef.current[target], direction, 0.025), 'keyboard')
      mixerRef.current.tick(direction)
      return true
    },
    [applyVolume]
  )
  ...
  const handleKey = useCallback(
    (e: KeyboardEvent): void => {
      ...
      if (condition === 'glanceshift' && mouseGazeFallback) {
        if (e.key === 'Enter') {
          if (!selectedTargetRef.current) return
          e.preventDefault()
          completeActiveCommand('glanceshift_keyboard_enter')
          return
        }
        if (e.key === 'q' || e.key === 'Q' || e.key === 'ArrowDown') {
          if (applyKeyboardVolumeStep('down')) e.preventDefault()
          return
        }
        if (e.key === 'e' || e.key === 'E' || e.key === 'ArrowUp') {
          if (applyKeyboardVolumeStep('up')) e.preventDefault()
          return
        }
        return
      }

      if (condition !== 'mouse-menu') return
    ...
    [applyKeyboardVolumeStep, completeActiveCommand, finishRun, logEvent, mouseGazeFallback, selectTarget]
  )
  ...
  useEffect(() => {
    if (condition !== 'glanceshift' || !selectedTarget || mouseGazeFallback) return
